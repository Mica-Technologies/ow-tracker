...
import com.micatechnologies.gaming.owtracker.AppCore;
...
import com.micatechnologies.gaming.owtracker.utilities.SystemUtilities;
import com.micatechnologies.minecraft.launcher.game.RuntimeManager;
...
public class OWTrackerSettingsGui extends OWTrackerAbstractGui
{
     ...
     */
    public OWTrackerSettingsGui(Stage stage ) throws IOException {
        super( stage );
    }
     ...
     */
    @SuppressWarnings( "unused" )
    public OWTrackerSettingsGui(Stage stage, double width, double height ) throws IOException {
    ...
    void setup() {
        ...
        returnBtn.setOnAction( actionEvent -> SystemUtilities.spawnNewTask( () -> {
            if ( dirty ) {
                ...
                if ( response == 1 ) {
                    ...
                    try {
                        OWTrackerGuiController.goToMainGui();
                    }
                    catch ( IOException e ) {
        ...
        saveBtn.setOnAction( actionEvent -> SystemUtilities.spawnNewTask( () -> {
            ...
            if ( ConfigConstants.ALLOWED_THEMES.contains( themeSelection.getSelectedValue() ) ) {
                ...
                OWTrackerGuiController.forceThemeRefresh();
            }
        ...
        resetRuntimeBtn.setOnAction( event -> SystemUtilities.spawnNewTask( () -> {
            ...
            try {
                OWTrackerGuiController.goToSettingsGui();
            }
            catch ( IOException e ) {
        ...
        String v = ApplicationConstants.LAUNCHER_APPLICATION_VERSION;
        ...
        double correctedMaxForMin = Math.min( ApplicationConstants.SETTINGS_MIN_RAM_MAX, maxRamGbVal );
        minRamGb.setValueFactory(
                new SpinnerValueFactory.DoubleSpinnerValueFactory( ApplicationConstants.SETTINGS_MIN_RAM_MIN,
                                                                   correctedMaxForMin, minRamGbVal, 0.1 ) );
        minRamGb.getValueFactory().valueProperty().addListener( ( observable, oldValue, newValue ) -> {
            setEdited( true );
            double newValWithMinForMax = Math.max( ApplicationConstants.SETTINGS_MAX_RAM_MIN, newValue );
        ...
        double correctedMinForMax = Math.max( ApplicationConstants.SETTINGS_MAX_RAM_MIN, minRamGbVal );
        maxRamGb.setValueFactory( new SpinnerValueFactory.DoubleSpinnerValueFactory( correctedMinForMax,
                                                                                     ApplicationConstants.SETTINGS_MAX_RAM_MAX,
                                                                                     maxRamGbVal, 0.1 ) );
        maxRamGb.getValueFactory().valueProperty().addListener( ( observable, oldValue, newValue ) -> {
            setEdited( true );
            double newValWithMaxForMin = Math.min( ApplicationConstants.SETTINGS_MIN_RAM_MAX, newValue );
