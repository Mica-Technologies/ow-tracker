...
import com.micatechnologies.gaming.owtracker.utilities.*;
...
public class OWTrackerMainGui extends OWTrackerAbstractGui
{
    /**
     * Mod pack logo image.
     *
     ...
     */
    public OWTrackerMainGui(Stage stage ) throws IOException {
        super( stage );
    }
     ...
     */
    @SuppressWarnings( "unused" )
    public OWTrackerMainGui(Stage stage, double width, double height ) throws IOException {
    ...
    void setup() {
        ...
        exitBtn.setOnAction( event -> AppCore.closeApp() );
        ...
        SystemUtilities.spawnNewTask( () -> {
            try {
                // Get current version
                String version = ApplicationConstants.LAUNCHER_APPLICATION_VERSION;
        ...
        editButton.setOnAction( actionEvent -> SystemUtilities.spawnNewTask( () -> {
            try {
                OWTrackerGuiController.goToEditModpacksGui();
                SystemUtilities.spawnNewTask(
        ...
        playBtn.setOnAction( actionEvent -> SystemUtilities.spawnNewTask( () -> {
            ...
            SystemUtilities.spawnNewTask( () -> DiscordRpcUtility.setRichPresence( "In Game (Minecraft)", "Mod Pack: " +
                                                                                   ...
                                                                                   "In Game" ) );
            AppCore.play( installedModPackByFriendlyName, () -> GUIUtilities.JFXPlatformRun( () -> {
                try {
                    Objects.requireNonNull( OWTrackerGuiController.getTopStageOrNull() ).show();
                    OWTrackerGuiController.goToMainGui();
                    OWTrackerGuiController.requestFocus();
                }
                catch ( Exception e ) {
